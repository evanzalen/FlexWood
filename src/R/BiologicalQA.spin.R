...
#' The samples match to the order of the file list. This was the case for this file list so the first
...
#' * add file list to samples as a new column
...
ggplot(dat, aes(x, y, fill = samples$Flexure)) + 
  geom_col() + 
  scale_y_continuous(name = "reads") +
  facet_grid(~ factor(samples$Flexure), scales = "free") +
  theme_bw() + 
...
nlevel = nlevels(dds$Tissue) * nlevels(dds$Flexure)
